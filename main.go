...
var Versions = struct {
	...
	tool: struct {
		...
		name:     "Bash",
		id:       "bash",
		versions: []string{"4.3", "4.4", "5.0", "5.1"},
	},
}

func writeBash() error {
	// by-distro
	for _, distro := range Versions.distributions {
		if err := os.MkdirAll(fmt.Sprintf("./containers/bash/by-distro/%s", distro.id), 0755); err != nil {
			return err
		}

		for _, distroVersion := range distro.versions {
			if err := os.WriteFile(
				fmt.Sprintf("./containers/bash/by-distro/%s/%s-%s.Containerfile", distro.id, distro.id, distroVersion),
				[]byte(fmt.Sprintf(`# THIS FILE IS AUTOGENERATED! DO NOT EDIT!
...
ENTRYPOINT [ "bash" ]
`, distro.id, distroVersion)), 0o755); err != nil {
				return err
			}
		}
	...
	// by-version
	for _, toolVersion := range Versions.tool.versions {
		if err := os.MkdirAll(fmt.Sprintf("./containers/bash/by-version/%s", "debian"), 0755); err != nil {
			return err
		}

		if err := os.WriteFile(
			fmt.Sprintf("./containers/bash/by-version/debian/debian-bash-%s.Containerfile", toolVersion),
			[]byte(fmt.Sprintf(`# THIS FILE IS AUTOGENERATED! DO NOT EDIT!
...
`, "debian", "12", toolVersion)), 0o755); err != nil {
			return err
		}
	}
